...
export default function HeroNotifications() {
  ...
  return (
    <div className="absolute -left-4 sm:-left-10 top-1/2 -translate-y-1/2 z-20 pointer-events-none" aria-live="polite">
      <AnimatePresence>
        {active && (
          <motion.div
            ...
            className="glass rounded-xl px-3.5 py-2.5 flex items-center gap-2.5 shadow-2xl shadow-black/40 max-w-[190px]"
          >
            <span className={`w-7 h-7 rounded-full bg-white/5 flex items-center justify-center flex-shrink-0 ${active.accent}`}>
              <active.icon className="w-3.5 h-3.5" />
            </span>
            <span className="text-[11px] text-white leading-tight">{active.text}</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
