...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none" role="banner">
      <div className={`transition-all duration-500 ${scrolled ? 'pt-3' : 'pt-5'}`}>
        <nav
          className={`pointer-events-auto mx-4 sm:mx-6 lg:mx-auto flex items-center justify-between transition-all duration-500 ${
            scrolled
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <a
                    key={link.href}
                    href={link.href}
                    className={`text-base transition-colors py-2.5 border-b border-white/5 last:border-0 flex items-center gap-2 ${
                      isActive ? 'text-accent font-semibold' : 'text-text-secondary hover:text-white'
                    }`}
