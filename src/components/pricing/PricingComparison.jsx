      ...
      <table className="w-full min-w-[600px]" role="table">
        <thead>
          <tr className="border-b border-white/[0.08]">
            <th className="text-left py-4 text-sm font-medium text-text-secondary w-1/4">Feature</th>
            <th className="text-center py-4 text-sm font-medium text-text-secondary w-1/4">Starter</th>
            <th className="text-center py-4 text-sm font-medium gradient-text w-1/4">Growth</th>
            <th className="text-center py-4 text-sm font-medium text-text-secondary w-1/4">Enterprise</th>
          </tr>
        </thead>
        <tbody>
          {features.map((f) => (
            <tr key={f.name} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
              <td className="py-3.5 text-sm text-white">{f.name}</td>
              <td className="py-3.5 text-center"><CellValue value={f.starter} /></td>
              <td className="py-3.5 text-center bg-accent/[0.02]"><CellValue value={f.growth} /></td>
              <td className="py-3.5 text-center"><CellValue value={f.enterprise} /></td>
            </tr>
          ))}
        </tbody>
      </table>
